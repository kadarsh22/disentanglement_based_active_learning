...
class infogancifar10_2class: ##TODO start wih infoganCifar2Helper
	def __init__(self, device, sample_size=1000, z_dim=66, len_discrete_code=2,active_learning =False):
		...
		self.G.load_state_dict(torch.load('utils/cifar10_2class/trained_models/infoGAN/infoGAN_G.pkl'))
		self.G.eval()
	...
	def generate_images(self,model = None):
		...
		y_cont_ = torch.from_numpy(np.random.uniform(-1, 1, size=(self.sample_size, 2))).type(torch.FloatTensor)
		y_disc_one = torch.Tensor([1, 0] * each_class_samples).view(-1, 2)
		y_disc_two = torch.Tensor([0, 1] * each_class_samples).view(-1, 2)
