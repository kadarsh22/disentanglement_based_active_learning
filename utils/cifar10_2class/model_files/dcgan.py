...
#######for cifar input_size  =32

parser = argparse.ArgumentParser()
parser.add_argument('--dataset', required=True, help='cifar2 | cifar10 | lsun | mnist |imagenet | folder | lfw | fake')
...
if opt.dataset in ['imagenet', 'folder', 'lfw']:
    ...
    horse_auto_valid_data = [dataset[i][0] for i in range(dataset.data.shape[0]) if dataset.targets[i] == 7 or dataset.targets[i] == 1]
    horse_auto_valid_data = torch.stack(horse_auto_valid_data)
    horse_auto_targets = dataset.targets[(np.array(dataset.targets) == 7) |  (np.array(dataset.targets) == 1)]
    
    horse_auto_targets[horse_auto_targets == 7] = 0
    horse_auto_targets[horse_auto_targets == 1] = 1
    dataset = NewDataset(horse_auto_valid_data, horse_auto_targets)
